...
$\mathbf{H = \sum_i^h \Gamma(A_i) \Gamma (A_i^\dagger)}$. Dann diese diagonalisieren mit unitärer Trafo 
$\mathbf(d) = \mathbf{U}^{-1} \mathbf{H} \mathbf{U}$. Somit ist
die Darstellung 
\begin{equation*}
  \Gamma^{''}(A_j) = \mathbf{d^{-\frac{1}{2}}} \mathbf{U}^{-1} \Gamma(A_j) \mathbf{U} d^{\frac{1}{2}}
\end{equation*}
unitär.
